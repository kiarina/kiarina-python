...
環境変数 prefix は `KIARINA_AGI_RUN_CONTEXT_` です。`RunContext` の作成時に識別子が未設定の場合、または `disallow_default_ids` が `True` で識別子が `default` の場合に `ValueError` を送出します。multi-user 環境ではこのフラグを有効にして、識別子の明示設定を強制できます。
