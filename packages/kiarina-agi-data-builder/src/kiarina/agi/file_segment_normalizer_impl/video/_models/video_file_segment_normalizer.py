...
class VideoFileSegmentNormalizer(BaseFileSegmentNormalizer):
    async def normalize_file_segments(
        self,
        file_infos: list[FileInfo],
        file_blob: FileBlob,
    ) -> list[FileInfo]:
        video_file_infos = [fi for fi in file_infos if isinstance(fi, VideoFileInfo)]

        video_file_infos.sort(key=lambda fi: fi.normalized_start_time)

        new_segments: list[VideoFileInfo] = []
        last_segment: VideoFileInfo | None = None
