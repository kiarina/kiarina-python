...
### Added
- Add a general-purpose `Memory` model for text, assets, vectors, and type-specific metadata.
- Add an in-memory `MemoryGraph` with permanent and decaying edges.

### Changed (BREAKING)
- Replace `History.embeddings` and its embedding management methods with `History.memory_graph`.
