...
def test_clear(text_file_info) -> None:
    history = History(
...
def test_add_event(history: History, text_file_info) -> None:
...
def test_get_file_info(text_file_info) -> None:
...
def test_get_file_infos_uri_or_file_path(text_file_info) -> None:
    history = History(file_infos=[text_file_info])
    assert (
...
def test_get_file_infos_group(text_file_info) -> None:
    text_file_info.group = "g1"
    history = History(file_infos=[text_file_info])
    assert len(history.get_file_infos(group="g1")) == 1


def test_get_file_infos_no_group(text_file_info) -> None:
...
def test_get_file_infos_no_unique_key(text_file_info) -> None:
    text_file_info.unique_key = "u1"
    history = History(file_infos=[text_file_info])
    assert len(history.get_file_infos(no_unique_key=True)) == 0


def test_get_file_infos_ignore_unique_keys(text_file_info) -> None:
    text_file_info.unique_key = "u1"
    history = History(file_infos=[text_file_info])
    assert len(history.get_file_infos(ignore_unique_keys=["u1"])) == 0


def test_get_file_infos_in_message(history: History, text_file_info) -> None:
    history.add_event(HumanMessageEvent.create("Hello", [text_file_info]))
    assert len(history.get_file_infos(in_message=True)) == 1
    assert len(history.get_file_infos(in_message=False)) == 0


def test_add_file_info(history: History, text_file_info) -> None:
...
def test_remove_file_info(history: History, text_file_info) -> None:
    history.add_file_info(text_file_info)
