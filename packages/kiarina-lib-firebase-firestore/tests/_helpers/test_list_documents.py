...
async def test_happy_path(
    ...
    result = await list_documents(
        project_id, f"users/{user_id}/items", id_token=id_token
    )
...
async def test_pagination(
    ...
    first_page = await list_documents(
        project_id, f"users/{user_id}/items", page_size=2, id_token=id_token
    )
...
async def test_empty_collection(
    ...
    result = await list_documents(
        project_id, f"users/{user_id}/empty", id_token=id_token
    )
