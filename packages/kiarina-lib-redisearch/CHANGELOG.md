...
### Fixed
- Type-check cleanly against redis-py 8.x, whose `hset()` and `hgetall()` signatures are stricter.
...
### Changed
- Update tests to read sample JSON files from shared `tests/assets` paths.
