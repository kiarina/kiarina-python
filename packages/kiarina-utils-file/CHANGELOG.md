...
### Added
- **MIME type detection**: Added `ambiguous_extensions` setting to handle ambiguous file extensions
  - Useful for extensions like `.ts` that could represent multiple MIME types (TypeScript vs MPEG-2 Transport Stream)
  - When a file extension is marked as ambiguous, extension-based detection is bypassed and content-based detection is used instead
  - Default list includes `.ts`
  - Can be customized via `KIARINA_UTILS_MIME_AMBIGUOUS_EXTENSIONS` environment variable or `options` parameter in `detect_mime_type()`
  - Added `is_ambiguous_extension()` operation function in `_operations` module for better code organization and testability
  - Settings merge logic is encapsulated within `is_ambiguous_extension()` for cleaner API
